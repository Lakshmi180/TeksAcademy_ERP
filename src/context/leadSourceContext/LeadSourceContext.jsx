...
const LeadSourceContextProvider = ({ children }) => {
    ...
    const [leadSourceState, DispatchLeadSource] = useReducer(LeadSourceReducer,initialState)
    ...
    const getAllLeadSource =async()=>{
        try{
            const {data, status}=await axios.get(`${process.env.REACT_APP_API_URL}/getleadsource`)
    ...
    return (
        <LeadSourceContext.Provider value={{leadSourceState, DispatchLeadSource, CreateleadSource,getAllLeadSource}}>
          {children}
