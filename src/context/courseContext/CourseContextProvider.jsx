...
const CourseContextProvider=({children})=>{
    ...
    const CreateCourse =async(coursedata)=>{
        try{
            const {data, status}=await axios.post(`${process.env.REACT_APP_API_URL}/createcourse`, coursedata)
            if(status==200){
    ...
    const UpdateCourse=async(updatedcoursedata)=>{
        try{
            const {data, status}=await axios.put(`${process.env.REACT_APP_API_URL}/updatecourse`, updatedcoursedata)
            if(status==200){
    ...
    return(
        <CourseContext.Provider value={{courseState,DispatchCourse,getAllCourses,CreateCourse,UpdateCourse }}>
            {children}
